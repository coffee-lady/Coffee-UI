local BoxNode = require('CoffeeUI.gui.core.nodes.box_node.box_node')
local TextNode = require('CoffeeUI.gui.core.nodes.text_node.text_node')
...
function AbstractButton:initialize(scenes_service, ids, on_click)
    ...
    self.nodes = {container = get_box_object(container_id)}
...
function AbstractButton:click(action)
    ...
    if is_picked then
        if action.pressed and not self.is_pressed then
            self:press()
            return
        end

        if action.released and self.is_pressed then
            self:release_with_action()
            return
        end
...
function AbstractButton:release_with_action()
    self.is_pressed = false
    self:do_action()
end
...
function AbstractButton:_post_click_msg(action, is_picked)
    if action.pressed and is_picked then
        self.scenes_service:post_to_go(BOOTSTRAP_URL, AbstractButton.BUTTON_CLICKED, {type_id = self.type_id})
    end
end
...
function AbstractButton:enable()
    ...
    if self.disabled_color and self._native_color then
...
function AbstractButton:disable(disable_color, color_map)
    self.disabled = true
    self.disabled_color = disable_color
