...
local Math = require('libs.tools.math.math')
...
function Scroller:initialize(node_view_port, options)
    ...
    -- OPTIONS
    self.deceleration_value = options.deceleration or 0.1
    self.scroll_elasticity = options.scroll_elasticity or 0.1

    if options.autoselection == nil then
        self.autoselection = true
    else
        self.autoselection = options.autoselection
    end

    if options.is_inertia == nil then
        self.is_inertia = true
    else
        self.is_inertia = options.is_inertia
    end

    self.is_horizontal = options.is_horizontal == nil and true or options.is_horizontal

    if options.is_elastic == nil then
        self.is_elastic = true
    else
        self.is_elastic = options.is_elastic
    end

    self.scroll_sensivity = options.scroll_sensivity or 1

    if options.is_snap == nil then
        self.is_snap = true
    else
        self.is_snap = options.is_snap
    end
    ...
    self.autoscroll_easing = options.autoscroll_easing or Easing.outSine
    ...
    self.autoscroll = AutoscrollHelper(self.autoscroll_easing)
