...
function Button:press()
    if not self:is_animation_running() then
        self:_set_animation_pressed()
        self:_set_animation_released()
...
function Button:release_with_action()
    if self.animation_pressed:is_running() then
        self.animation_pressed:add(self.animation_released):add_callback(
            function()
                AbstractButton.release_with_action(self)
            end
        )
    end

    if not self:is_animation_running() then
        self.animation_released:add_callback(
            function()
                AbstractButton.release_with_action(self)
            end
        ):run()
...
function Button:is_animation_running()
    return self.animation_released:is_running() or self.animation_pressed:is_running()
end
